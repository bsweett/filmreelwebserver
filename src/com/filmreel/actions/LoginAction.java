...
public class LoginAction extends ActionSupport implements ServletRequestAware {
	...
	public String execute() throws Exception 
	{
		...
		try {
			...
			//Check to make sure that the user name parameter and email parameter are not empty	
			if(username.isEmpty() || password.isEmpty()) 
			{
				...
				System.out.println("ACTION :: Login - Returned fail (username or password parameter is empty)");
				return "fail"; 
			}
			
			manager = HibernateUserManager.getDefault();
	
			user = manager.getUserByNameAndPassword(username, password);
			
			//Check to make sure that the user name and password match an existing user
			if(user == null) 
			{
				messageStore.appendToMessage(XML);
				messageStore.appendToMessage(XML_DATA);
				...
				System.out.println("ACTION :: Login - The user could not be found.");
				return "NoUserFound";
			}
			
			//If all is good let the user log in
			else 
			{
				user.incramentCount();
				...
				System.out.println("ACTION :: Login - User login successful");
				return "success";
			}
		} 
		...
		catch (Exception e)
		{
			...
			System.out.println("ACTION :: Login - Fatal error occured.");
			return "error";
		}
	}	
